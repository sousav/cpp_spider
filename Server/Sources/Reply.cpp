...
namespace status_strings {
  ...
  boost::asio::const_buffer to_buffer(http::Reply::status_type status) {
    switch (status)
    {
    case http::Reply::ok:
...
std::vector<boost::asio::const_buffer> http::Reply::to_buffers()
{
...
namespace stock_replies {
  ...
  std::string to_string(http::Reply::status_type status)
  {
    switch (status)
    {
    case http::Reply::ok:
...
http::Reply http::Reply::stock_reply(http::Reply::status_type status) {
  http::Reply rep;
  rep.status = status;
...
http::Reply http::Reply::json_reply(const std::string &json) {
  return http::Reply::json_reply(http::Reply::status_type::ok, json);
}

http::Reply http::Reply::json_reply(http::Reply::status_type status, const std::string &json) {
  http::Reply rep;
  rep.status = status;
  rep.content = json;
  rep.headers.resize(3);
